...
4. **Ningún bloque de evidencia final se redacta de memoria — siempre se ensambla concatenando archivos ya volcados por la terminal.** Incidente real durante la Tarea 4.1: un agente presentó un log de Maven y un `git diff` que parecían legítimos pero contenían contenido "autocompletado" a partir de patrones típicos (una línea de copia de recursos que no ocurrió, dos excepciones distintas con el mismo hash de blob de Git) — reconstruidos de memoria en lugar de leídos literalmente. La corrección obligatoria para todo el resto del backlog: cada comando cuya salida deba citarse se redirige primero a un archivo (`comando > /tmp/archivo.log 2>&1`), y el bloque de evidencia se arma únicamente con `cat` de ese archivo — nunca retipeando ni resumiendo lo que "se vio en pantalla". Si una herramienta no puede garantizar este flujo (por ejemplo, porque solo expone el texto ya procesado de una respuesta anterior), debe decirlo explícitamente en vez de reconstruir el contenido.
   ...
   **Precisión sobre comandos de larga duración (aclarada tras la Tarea 4.10):** algunas herramientas agénticas tienen un límite técnico real (ej. ~10 segundos) antes de que un comando que no ha terminado se envíe automáticamente a background, fuera del control del agente. Esto por sí solo NO viola la regla de evidencia — un comando como `mvn clean test` sobre el reactor completo puede legítimamente tardar varios minutos y verse forzado a background por ese límite. Lo que sigue siendo obligatorio, sin excepción: (a) declarar explícitamente, en el momento en que ocurre, que el comando pasó a background por ese límite — no descubrirlo después por el resultado ni presentarlo como si se hubiera observado en tiempo real; (b) una vez el comando termina, el resultado se recupera y se cita LITERALMENTE desde el archivo de log (`cat /tmp/archivo.log`), nunca resumido, parafraseado, ni presentado con lenguaje que sugiera observación directa en tiempo real ("acabo de ver que...", "el proceso terminó exitosamente" sin pegar el log). El background en sí no es la falta; presentar su resultado sin la misma evidencia literal que exige cualquier otro comando sí lo es.

5. **Antes de cualquier `git add` selectivo, ejecutar `git status` primero y reportar explícitamente cualquier archivo untracked que no vaya a incluirse en el commit, con una línea explicando por qué se excluye.** Incidente real durante la Tarea 4.9: un archivo `TransientErrorTest.java`, creado como scratch de exploración durante la Tarea 4.8, quedó sin trackear y sin reportar porque el `git add` de esa tarea listó archivos específicos sin que nadie verificara antes si quedaba algo más en el árbol de trabajo. El archivo permaneció invisible durante toda la Tarea 4.9 hasta que un `git add .` indiscriminado (también prohibido — ver siguiente punto) lo sacó a la luz por accidente. La verificación de `git status` antes del `add` selectivo es la única forma de que un residuo así no pase desapercibido entre tareas.
6. **Nunca usar `git add .` ni `git add -A` para capturar evidencia de un diff de archivos nuevos (untracked).** Si necesitas ver el diff completo de archivos nuevos antes de comitear, usa `git add <ruta1> <ruta2> ...` listando explícitamente cada archivo que sí pertenece a la tarea, o usa `git diff --no-index /dev/null <archivo>` para inspeccionar un archivo nuevo individual sin tocar el índice. `git add .`/`-A` agrega indiscriminadamente cualquier archivo presente en el árbol de trabajo, incluyendo residuos de tareas anteriores o archivos generados por error — exactamente el mecanismo que originó tanto el incidente de `target/` trackeado (Tarea 4.0) como el de `TransientErrorTest.java` (Tarea 4.9).
7. **Higiene de `target/` — verificación incompleta detectada en la Tarea 4.10:** el `chore/purge-tracked-build-artifacts` (ejecutado entre las Tareas 4.5 y 4.6) purgó `core/target/` y `crypto/target/`, pero un `git ls-files | grep -E '(^|/)target/'` ejecutado al cierre de la Tarea 4.10 encontró que `ai/target/`, `app/target/` y `contracts/target/` (46 archivos) seguían trackeados — el diagnóstico original nunca verificó el árbol completo del repositorio, solo los dos módulos que ya se sabía que estaban afectados. Pendiente: un segundo `chore/` que repita la purga sobre estos tres módulos, verificando el árbol completo (`git ls-files | grep target/` sin acotar a ningún módulo específico) antes de darlo por resuelto.
